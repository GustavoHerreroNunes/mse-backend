...
@tasks_survey_bp.route('/boarding/comments/task/<int:task_id>', methods=['GET'])
def get_all_comments_from_task(task_id):
    ...
    try:
        ...
        comments = current_session.execute(
            text("SELECT * FROM tbl_comment_survey_boarding WHERE id_task = :task_id ORDER BY section_index"),
            {"task_id": task_id}
        )
