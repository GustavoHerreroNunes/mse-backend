...
@tasks_survey_bp.route('/boarding/<int:task_id>', methods=['PUT'])
def update_boarding_task(task_id):
    """Update a boarding task (id_survey cannot be changed)"""
    try:
        ...
        # id_survey should not be updated
        if data.get("id_survey"):
            return jsonify({"error": "Cannot update id_survey field"}), 400
