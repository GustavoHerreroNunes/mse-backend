...
@tasks_survey_bp.route('/boarding/bollards/<int:task_id>', methods=['PUT'])
def update_bollards_qtd(task_id):
    """
    Update the fields `num_bollards_fwd` and `num_bollards_aft` from a boarding task.
    ...
    try:
        ...
        if "num_bollards_fwd" not in data:
            return jsonify({"error": "The field num_bollards_fwd is required for update"}), 400
        if "num_bollards_aft" not in data:
            return jsonify({"error": "The field num_bollards_aft is required for update"}), 400
