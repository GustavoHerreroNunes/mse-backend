...
from .utils.ai_cargo_condition import ai_cargo_condition
from querys.get_dados_cargo_type import get_dados_cargo_type
from querys.get_dados_cargo_condition import get_dados_cargo_condition
...
def draw_cargos_page(c, width, height, demanda_id, table_data):
    ...
    subtitle = "The inspected cargo to be transported by captioned vessel consist of:"
    current_y = draw_paragraph(subtitle, title_y - 30, "Calibri")

    dados, erro = get_dados_cargo_type(demanda_id)

    if erro:
        current_y = draw_paragraph(f"⚠️ Error loading cargo data: {erro}", current_y - 10, "Calibri", x=30)
        return

    for cargo_type, qtd in dados:
        qtd_num = int(qtd)
        qtd_str = num2words(qtd_num).capitalize()
        plural_suffix = '' if qtd_num == 1 else 's'
        bullet_text = f"• {qtd_str} ({qtd_num}) {cargo_type}{plural_suffix}."
        current_y = draw_paragraph(bullet_text, current_y - 10, "Calibri", x=40)

    subtitle = "6.1\tGENERAL CONDITION OF CARGOS"
    current_y = draw_paragraph(subtitle, current_y - 20, "CooperBlack") - 20

    subtitle = "Following sections show the inspection finds/notes regarding overall observed condition of the cargos."
    ...
    for idx, row in enumerate(table_data, 1):
        ...
        section_number = f"6.1.{idx}"
        header_line = f"{section_number} {cargo_type.upper()} #{idx} ({weight} T)"

        data, erro = get_dados_cargo_condition(cargo_id, cargo_type)
        if erro:
            comments = ["No further comments to be made."]
        else:
            comments = ai_cargo_condition(data)
        ...
        photo_url, error_packing = get_dados_photos(demanda_id, 3, 0, cargo_id)
        if error_packing:
            photo_url = [["-", "-", "-", "-", "-", "-"]]
        ...
        try:
            ...
            if len(imagens_salvas) == 1:
                ...
                for i, img_path in enumerate(imagens_salvas):
                    ...
                    if is_single_in_last_row:
                        # Centraliza a última imagem sozinha na linha
            ...
            comment_result = get_dados_comments(demanda_id, 3, 0, cargo_id)
