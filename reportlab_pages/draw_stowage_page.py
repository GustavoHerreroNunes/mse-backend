...
from querys.get_dados_ia_stowage import get_dados_ia_stowage
...
from .utils.ai_stowage_condition import ai_stowage_condition

def draw_stowage_page(c, width, height, demanda_id, table_data, figure_number, cod_atividade):
    ...
    if cod_atividade == "2":
        title = "9. STORAGE AREA"
        number = "9"
    else:
        title = "10. STOWAGE AREA"
        number = "10"
    ...
    subtitle = "Following pictures of cargo at storage area:"
    ...
    for idx, row in enumerate(table_data, 1):
        ...
        section_number = f"{number}.1.{idx}"
        header_line = f"{section_number} {cargo_type.upper()} #{idx} ({weight} T)"

        data, erro = get_dados_ia_stowage(cargo_id)
        if erro:
            comments = ["No further comments to be made."]
        else:
            comments = ai_stowage_condition(data, "Storage" if number == "9" else "Stowage")
        ...
        photo_url, error_packing = get_dados_photos(demanda_id, 3, 3, cargo_id)
        if error_packing:
            photo_url = [["-", "-", "-", "-", "-", "-"]]
        ...
        try:
            ...
            if len(imagens_salvas) == 1:
                ...
                caption_text = f'Figure {figure_number} – Overview of cargos at storage area {cargo_type} #{idx}.'
            ...
            comment_result = get_dados_comments(demanda_id, 3, 3, cargo_id)
