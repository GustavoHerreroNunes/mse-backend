...
from .utils.ai_lifting_condition import ai_lifting_condition
from querys.get_dados_lifting_condition import get_dados_lifting_condition
...
def draw_lifting_page(c, width, height, demanda_id, table_data, figure_number):
    ...
    title = "7. LIFTING APPLIANCES & EQUIPMENT"
    title_font = "CooperBlack"
    title_size = 20
    title_y = height - 150
    c.setFont(title_font, title_size)
    c.drawString(30, title_y, title)
    print("lifting items: ", table_data)
    margin_x = 30
    ...
    # Próximo título
    subtitle = "7.1 GENERAL CONDITION OF LIFTING APPLIANCES"
    current_y = draw_paragraph(subtitle, current_y, "CooperBlack") - 20
    subtitle = "Following sections show the inspection finds/notes regarding overall observed condition of the lifting appliances."
    ...
    for idx, row in enumerate(table_data, 1):
        quantity_items, description_items, capacity_items, length_items, id_items = row

        section_number = f"7.1.{idx}"
        header_line = f"{section_number} {quantity_items}X {description_items.upper()} ({capacity_items} TON)"

        data, erro = get_dados_lifting_condition(id_items, description_items)
        if erro:
            comments = [
                "Overall, the lifting material was determined to be in satisfactory operational condition.",
            ]
        else:
            comments = ai_lifting_condition(data, description_items)
        ...
        photo_url, error_packing = get_dados_photos(demanda_id, 3, 1, None, id_items)
        if error_packing:
            photo_url = [["-", "-", "-", "-", "-", "-"]]
        ...
        try:
            ...
            if len(imagens_salvas) == 1:
                ...
                caption_text = f'Figure {figure_number} – Lifting materials #{idx}.'
                ...
                for i, img_path in enumerate(imagens_salvas):
                    ...
                    # Centraliza a imagem se for a última sozinha
            ...
            comment_result = get_dados_comments(demanda_id, 3, 1, None, id_items)
